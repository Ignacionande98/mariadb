...
app.post("/todo", async (req, res) => {
  ...
  try {
    ...
    const response = await conn.query(
      ...
      [req.body.name, req.body.description, req.created_at, updated_at, req.body.status]
    );
...
app.put("/todo/:id", async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    const response = await conn.query(
      `UPDATE todo SET name=?, description=?, created_at=?, updated=?, status=? WHERE id=?`,
      [req.body.name, req.body.description, req.created_at, updated_at, req.body.status, req.params.id]
    );
